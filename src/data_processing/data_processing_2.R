...
source("src/data_processing/local_district_listing_helpers.R")
...
data_processing_2_function <- function() {
  ...
  log_info("dp3 - processing GPS Household data")
  log_info("dp3.1 - processing ICT data")
  ict_result <- process_gps_household_data(
    census_data = mphc_rbind,
  ...
  output_df <- ict_result$output_df
  ict_stats <- ict_result$assigned_hh_stats


  log_info("dp3.2 - processing IHS6 data")
  IHS_result <- process_gps_household_data(
    census_data = output_df,
  ...
  output_df <- IHS_result$output_df
  IHS_stats <- IHS_result$assigned_hh_stats

  log_info("dp3.3 - processing NACA data")
  ...
  output_df <- Naca_result$output_df
  Naca_stats <- Naca_result$assigned_hh_stats

  log_info("dp4 - processing DHS listing data")
  ...
  log_info("dp5- writing transformation stats")
  ...
  log_info("dp6 - processing DHS survey data")
  ...
  log_info("dp7 - processing Zomba data")
  ...
  log_info("dp8 - processing Malemia data")
  ...
  log_info("dp9 - deriving observed household counts")
  output_df <- output_df %>%
    mutate(
  ...
  log_info("dp11 - creating household size geopackage")
  hh_size <- output_df %>%
